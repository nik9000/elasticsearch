 ...
 */
public final class SpatialExtentCartesianShapeCombinedDocValuesAggregatorFunction implements AggregatorFunction {
  ...
  private void addRawBlock(DoubleBlock valuesBlock) {
    for (int p = 0; p < valuesBlock.getPositionCount(); p++) {
      SpatialExtentCartesianShapeCombinedDocValuesAggregator.combine(state, p, valuesBlock);
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, DriverContext driverContext) {
    blocks[offset] = SpatialExtentCartesianShapeCombinedDocValuesAggregator.evaluateFinal(state, driverContext);
  }

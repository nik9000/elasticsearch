...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link TopBytesRefAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class TopBytesRefGroupingAggregatorFunction implements GroupingAggregatorFunction {
  private static final List<IntermediateStateDesc> INTERMEDIATE_STATE_DESC = List.of(
      new IntermediateStateDesc("top", ElementType.BYTES_REF)  );

  private final TopBytesRefAggregator.GroupingState state;
  ...
  TopBytesRefGroupingAggregatorFunction(List<Integer> channels, DriverContext driverContext,
      int limit, boolean ascending) {
    this.limit = limit;
    this.ascending = ascending;
    this.channels = channels;
    this.state = TopBytesRefAggregator.initGrouping(driverContext.bigArrays(), limit, ascending);
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int vOffset = vStart; vOffset < vEnd; vOffset++) {
          ...
          TopBytesRefAggregator.combine(state, groupId, vValue);
        }
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext evaluationContext) {
    blocks[offset] = TopBytesRefAggregator.evaluateFinal(state, selectedInPage, evaluationContext);
  }

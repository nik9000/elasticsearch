...
public final class SampleIntAggregatorFunction implements AggregatorFunction {
  ...
  SampleIntAggregatorFunction(DriverContext driverContext, List<ExpressionEvaluator> inputs,
      int limit) {
    this.limit = limit;
    this.driverContext = driverContext;
    this.inputs = inputs;
    this.state = SampleIntAggregator.initSingle(driverContext.bigArrays(), limit);
  }
  ...
  private void addRawInputMasked(Page page, BooleanVector mask) {
    try (IntBlock valueBlock = (IntBlock) inputs.get(0).eval(page)) {

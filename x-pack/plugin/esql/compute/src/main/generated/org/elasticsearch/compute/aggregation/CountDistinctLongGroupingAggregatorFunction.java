...
public final class CountDistinctLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int vOffset = vStart; vOffset < vEnd; vOffset++) {
          ...
          CountDistinctLongAggregator.combine(state, groupId, vValue);
        }
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        CountDistinctLongAggregator.combineIntermediate(state, groupId, hll.getBytesRef(valuesPosition, hllScratch));
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext evaluationContext) {
    blocks[offset] = CountDistinctLongAggregator.evaluateFinal(state, selectedInPage, evaluationContext);
  }

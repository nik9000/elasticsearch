...
public abstract class AbstractFixedTopNEncoderWithPagedTests extends AbstractSortableTopNEncoderWithPagedTests {
    @ParametersFactory
    public static Iterable<Object[]> parameters() {
        List<TestCase<?>> tests = new ArrayList<>();
        tests.add(testCase("ip", AbstractFixedTopNEncoderWithPagedTests::randomIp));
        tests.add(testCase("ip inside garbage", () -> embedInRandomBytes(randomIp())));
        return tests.stream().map(t -> new Object[] { t }).toList();
    }

    protected AbstractFixedTopNEncoderWithPagedTests(TestCase<?> testCase) {
        super(testCase);
    }

...
**Add `@Nullable WarningsSink warnings` to `DriverContext`.** Each driver is
single-threaded, so no concurrent collection is needed. `WarningsSink` wraps a
`LinkedHashSet<String>` to give deduplication and insertion-order preservation.
A non-null sink means "collect warnings" (was `COLLECT`), a null sink means
"ignore warnings" (was `IGNORE`). The old `WarningsMode` enum is removed.
`warningsMode()` is renamed to `warnings()` returning `@Nullable WarningsSink`.

When `DriverContext.finish()` is called, `WarningsSink.takeWarnings()` is captured
into `DriverContext.Snapshot` as an unmodifiable `Set<String>`. A method on
`Snapshot` -- `dumpWarningsToThreadContext()` -- iterates the set and calls
...
**Pass the `@Nullable WarningsSink` into `Warnings`.** Change
`Warnings.createWarnings(warningsMode, source)` to
`Warnings.createWarnings(sink, source)` where `sink` is the
`@Nullable WarningsSink`. If null, return `NOOP_WARNINGS`. Otherwise construct a
`Warnings` that appends to the sink instead of calling
`HeaderWarning.addWarning()`.

1. **`Warnings` writes to the sink.** `registerException()` formats the warning
   string exactly as before, then appends it to the sink instead of calling
   `HeaderWarning.addWarning()`.

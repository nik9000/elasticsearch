 ...
 */
public final class SpatialWithinGeoSourceAndConstantEvaluator implements ExpressionEvaluator {
  ...
  public SpatialWithinGeoSourceAndConstantEvaluator(Source source, ExpressionEvaluator left,
  ...
  public BooleanBlock eval(int positionCount, BytesRefBlock leftBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          SpatialWithin.processGeoSourceAndConstant(result, p, leftBlock, this.right);
  ...
  public String toString() {
    return "SpatialWithinGeoSourceAndConstantEvaluator[" + "left=" + left + ", right=" + right + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public SpatialWithinGeoSourceAndConstantEvaluator get(DriverContext context) {
      return new SpatialWithinGeoSourceAndConstantEvaluator(source, left.get(context), right, context);
    }

 ...
 */
public final class EqualsMillisNanosEvaluator implements ExpressionEvaluator {
  ...
  public EqualsMillisNanosEvaluator(Source source, ExpressionEvaluator lhs, ExpressionEvaluator rhs,
  ...
  public BooleanBlock eval(int positionCount, LongBlock lhsBlock, LongBlock rhsBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(Equals.processMillisNanos(lhs, rhs));
      }
      return result.build();
    }
  }
  ...
  public BooleanVector eval(int positionCount, LongVector lhsVector, LongVector rhsVector) {
    try(BooleanVector.FixedBuilder result = driverContext.blockFactory().newBooleanVectorFixedBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(p, Equals.processMillisNanos(lhs, rhs));
      }
      return result.build();
    }
  }

 ...
 */
public final class NotEqualsGeometriesEvaluator implements ExpressionEvaluator {
  ...
  public NotEqualsGeometriesEvaluator(Source source, ExpressionEvaluator lhs,
  ...
  public BooleanBlock eval(int positionCount, BytesRefBlock lhsBlock, BytesRefBlock rhsBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(NotEquals.processGeometries(lhs, rhs));
      }
      return result.build();
    }
  }
  ...
  public BooleanVector eval(int positionCount, BytesRefVector lhsVector, BytesRefVector rhsVector) {
    try(BooleanVector.FixedBuilder result = driverContext.blockFactory().newBooleanVectorFixedBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(p, NotEquals.processGeometries(lhs, rhs));
      }
      return result.build();
    }
  }

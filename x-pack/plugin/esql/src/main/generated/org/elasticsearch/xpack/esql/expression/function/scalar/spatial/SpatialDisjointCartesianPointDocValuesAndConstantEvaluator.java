 ...
 */
public final class SpatialDisjointCartesianPointDocValuesAndConstantEvaluator implements ExpressionEvaluator {
  ...
  public BooleanBlock eval(int positionCount, LongBlock leftBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          SpatialDisjoint.processCartesianPointDocValuesAndConstant(result, p, leftBlock, this.right);
  ...
  public String toString() {
    return "SpatialDisjointCartesianPointDocValuesAndConstantEvaluator[" + "left=" + left + ", right=" + right + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public SpatialDisjointCartesianPointDocValuesAndConstantEvaluator get(DriverContext context) {
      return new SpatialDisjointCartesianPointDocValuesAndConstantEvaluator(source, left.get(context), right, context);
    }

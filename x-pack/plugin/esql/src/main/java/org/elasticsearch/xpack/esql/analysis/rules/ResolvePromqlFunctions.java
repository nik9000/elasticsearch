...
public class ResolvePromqlFunctions extends ParameterizedAnalyzerRule<PromqlCommand, AnalyzerContext> {
    ...
    static LogicalPlan resolveFunction(UnresolvedPromqlFunction unresolved) {
        ...
        if (grouping != null) {
            ...
            return new AcrossSeriesAggregate(unresolved.source(), child, extraParams, metadata, grouping, unresolved.groupingKeys());
        }
        ...
        return switch (metadata.functionType()) {
            ...
            case WITHIN_SERIES_AGGREGATION -> new WithinSeriesAggregate(unresolved.source(), child, extraParams, metadata);
            case VALUE_TRANSFORMATION -> new ValueTransformationFunction(unresolved.source(), child, extraParams, metadata);
            case VECTOR_CONVERSION -> new VectorConversionFunction(unresolved.source(), child, extraParams, metadata);
            case SCALAR_CONVERSION -> new ScalarConversionFunction(unresolved.source(), child, extraParams, metadata);
            case SCALAR, TIME_EXTRACTION -> child == null
                ? new ScalarFunction(unresolved.source(), name)
                : new ValueTransformationFunction(unresolved.source(), child, extraParams, metadata);

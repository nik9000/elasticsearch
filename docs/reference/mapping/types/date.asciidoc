...
<<relocate_to>>::
    ...
    but for dates specifically, setting `relocate_to` to `doc_values` will
    cause the date to always be returned as a string formatted with the
    `format` and is only allowed if the `format` prints the date with enough
    precision to fully rebuild it. For example, `strict_date_optional_time`,
    `yyyy.MM.dd'T'HH:mm:ss:SSSZ`, and the default are all fine but
    `epoch_second` isn't because it doesn't contain the milliseconds field.

...
single action does not affect the remaining actions. You can save a
few bytes by specifying the `terse` URL parameter and Elasticsearch
will skip returning the large JSON structure with the individual
results if all the results are successful. Any failure will cause it
to return the entire JSON structure.
